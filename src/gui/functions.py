...
class Funcs:
    ...
    def csv_to_dataframe(
            ...
            input_file: str, 
            sep: str,
            engine: str) -> Tuple[pd.DataFrame, str]:
    ...
    def plot(self, df, label, ax, state='Add') :
    ...
    def load(self) -> None:
        ...
        try:
            ...
            self.undo_stack.append(('Load', last, new, old_x_lim, 
                                    old_y_lim, new_x_lim, new_y_lim))
        except Exception as e:
            raise CustomException(e)

    def add_spectrum(self, curve: Line2D) -> Tuple[Spectrum, int]:
    ...
    def add_plot(self) -> None:
        ...
        try:
            ...
            df, label = self.csv_to_dataframe(input_file=input_file,
                                              sep=self.sep,
                                              engine=self.engine)
            ...
            self.undo_stack.append(('Add Plot', prev, new, old_x_lim, 
                                    old_y_lim, new_x_lim, new_y_lim))
    ...
    ## Data processing ##
    def process_data(self, function, params):
        """A function that calls data processing functions
          to the Spectrum objects that are visible (checked)"""
        try:
            for i in self.curves.values():
                if i.tristate == 1:
