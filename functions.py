...
# canvas
    def add_cursor(self,ax,hl=True,vl=True,color='red',lw=0.8):
        self.cursor = Cursor(
                        ax,
                        horizOn=hl,
                        vertOn=vl,
    ...
    def csv_to_dataframe(self, input_file):
        try:
            label = input_file.split('/')[-1].replace('.csv','')
            df = pd.read_csv(input_file, 
                            sep=self.add_setting("General.sep"),
                            dtype='float')
            return df, label
        except Exception as e:
            print(e)
    ...
    def plot(self, df, label, state='Add'):
        try:
            ax = df.plot(
                ...
                x=0,
                y=1,
                lw=self.add_setting("General.lw"),
                legend=self.legend,
                label=label)
...
# data processing
            ...
            for i in self.curves.values():
                if i.tristate == 1:
                    ...
                    i.y_smooth = savgol_filter(
                                    i.y(),
                                    self.add_setting("Smooth.window_length"),
                                    self.add_setting("Smooth.polyorder"),
                                    self.add_setting("Smooth.deriv"),
                                    self.add_setting("Smooth.delta"),
                                    self.add_setting("Smooth.axis"))
                    i.y_data = i.y_smooth
        ...
        try:
            for i in self.curves.values():
                if i.tristate == 1:
                    ...
                    peaks, _ = find_peaks(
                                i.y(),
                                height=self.add_setting("Peaks.height"),
                                threshold=self.add_setting("Peaks.threshold"),
                                distance=self.add_setting("Peaks.distance"),
                                prominence=self.add_setting("Peaks.prominence"),
                                width=self.add_setting("Peaks.width")
                                )
        ...
        try:
            for i in self.curves.values():
                if i.tristate == 1:
                    prev = np.copy(i.y_data)
                    y_ = i.y_data
                    lam = self.add_setting("Baseline.lam")
                    p = self.add_setting("Baseline.p")
                    niter = self.add_setting("Baseline.niter")
                    L = len(y_)
    ...
    def edit_form(self):
        try:
            ...
            for i in self.curves.values():
                if i.tristate == 1:
                    ax.plot(i.x(), i.y(), lw=self.add_settings("lw"), label=i.label())
        ...
        try:
            for i in self.curves.values():
                if i.tristate == 1:
                    ...
                    self.undo_stack.append(('Normalize Min-Max', prev, i.y_normalized, i))
            self.canvas_update()
        except Exception as e:
            print(e)
...
# Spinbox - Checkbox
    def chkbox_clicked(self,row,col):
    ...
    def spinbox_value_changed(self, axis):
...
# Tables
    ...
    def update_spectrum_table(self):
        try:
            ...
            for i,j in enumerate(self.curves.values()):
                ...
                if not j.label().startswith('peak_'):
                    ...
                    chkBoxItem.setFlags(QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsUserTristate | QtCore.Qt.ItemIsEnabled)
...
# Settings Window
    def open_settings(self):
        try:
            self.settings.lineEdit_title.setText(self.title)

            x_bounds = self.canvas.axes.get_xlim()
            self.settings.lineEdit_xaxis_left.setText(str(x_bounds[0]))
            self.settings.lineEdit_xaxis_right.setText(str(x_bounds[1]))
            self.settings.lineEdit_xaxis_label.setText(self.xlabel)

            y_bounds = self.canvas.axes.get_ylim()
            self.settings.lineEdit_yaxis_bottom.setText(str(y_bounds[0]))
            self.settings.lineEdit_yaxis_top.setText(str(y_bounds[1]))
            self.settings.lineEdit_yaxis_label.setText(self.ylabel)

            self.settings.update_dropdown(self.curves)

            self.settings.show()
        except Exception as e:
            print(e)

    def settings_closed(self, object):
        self.df_settings = object # change the settings

# Default settings
    def read_json(self, json):
        try:
            return pd.read_json(json).fillna(np.nan).replace([np.nan],[None])
        except Exception as e:
            print(e)

    def add_setting(self,label):
        try:
            return self.df_settings[label].values[0]
        except Exception as e:
            print(e)
...
# Save spectrum
    ...
    def check_path(self,file, title):
